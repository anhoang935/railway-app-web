  ...
  static async update(scheduleID, scheduleData) {
    try {
      ...
      const [result] = await pool.query(
        ...
        [
          start_stationID || scheduleCheck[0].start_stationID,
          end_stationID || scheduleCheck[0].end_stationID,
          departureTime || scheduleCheck[0].departureTime,
          arrivalTime || scheduleCheck[0].arrivalTime,
          scheduleStatus || scheduleCheck[0].scheduleStatus,
          trainID || scheduleCheck[0].trainID,
          scheduleID
        ]
      );
